...
//                  ErrMsg(TEXT("%x: Cannot set frame rate or resolution for preview"), hr);
    ...
    //
    {
        ...
        if(SUCCEEDED(hr))
        {
            ...
            if(hr == NOERROR)
            {
                lstrcpyW(wachAudioFriendlyName, var.bstrVal);
                SysFreeString(var.bstrVal);
            }
...
void UpdateStatus(BOOL fAllStats)
{
    ...
    if(!fAllStats)
    {
        ...
        wsprintf(tach, TEXT("Captured %d frames (%d dropped) %d.%dsec\0"), lNot,
                 lDropped, lTime / 1000, 
...
#define DESCSIZE 80
    ...
    for(i = 0; i < NUMELMS(gcap.rgpmAudioMenu); i++)
    {
        ...
        CheckMenuItem(GetMenu(ghwndApp), MENU_ADEVICE0 + i, 
            (gcap.pmAudio && S_OK == gcap.rgpmAudioMenu[i]->IsEqual(gcap.pmAudio)) ? MF_CHECKED : MF_UNCHECKED);
    }
...
#else
    wcsncpy(wszVideo, T2W(szVideo), NUMELMS(wszVideo)-1);
    wcsncpy(wszAudio, T2W(szAudio), NUMELMS(wszAudio)-1);
#endif
        ...
        case WM_COMMAND :
            switch(GET_WM_COMMAND_ID(wParam, lParam))
            {
                ...
                case IDD_SetCapFileSize:
                {
                    ...
                    if(bchanged)
                    {
                        wsprintf(tachBuffer, TEXT("%ld\0"), l);
...
int FAR PASCAL FrameRateProc(HWND hwnd, UINT msg, UINT wParam, LONG lParam)
{
    ...
    switch(msg)
    {
        case WM_INITDIALOG:
            ...
            wsprintf(tach, TEXT("%d\0"), (int) gcap.FrameRate);
...
int FAR PASCAL TimeLimitProc(HWND hwnd, UINT msg, UINT wParam, LONG lParam)
{
    ...
    switch(msg)
    {
        case WM_INITDIALOG:
            ...
            wsprintf(tach, TEXT("%d\0"), gcap.dwTimeLimit);
...
int FAR PASCAL PressAKeyProc(HWND hwnd, UINT msg, UINT wParam, LONG lParam)
{
    ...
    switch(msg)
    {
        case WM_INITDIALOG:
            ...
            wsprintf(tach, TEXT("%s\0"), gcap.szCaptureFile);
...
void OnClose()
{
    ...
    WriteProfileString(TEXT("annie"), TEXT("CaptureFile"), gcap.szCaptureFile);
